...
Vagrant.configure("2") do |config|
  ...
  #
  # By default, we'll include the domains set up by cividev through the cividev-hosts file
  ...
  #
  config.vm.network :private_network, id: "cividev_primary", ip: "192.168.123.10"

  config.vm.provider :hyperv do |v, override|
    override.vm.network :private_network, id: "cividev_primary", ip: nil
  end
  ...
  if vagrant_version >= "1.3.0"
    config.vm.synced_folder "www/", "/srv/www/", :owner => "www-data", :mount_options => [ "dmode=775", "fmode=664" ]
  else
  ...
  config.vm.provider :hyperv do |v, override|
    override.vm.synced_folder "www/", "/srv/www/", :owner => "www-data", :mount_options => ["dir_mode=0775","file_mode=0664","forceuid","noperm","nobrl","mfsymlinks"]
